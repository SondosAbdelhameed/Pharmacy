...
<div class="radio">
  <label><input type="radio" name="patient_type" value="0" onclick="current_patient_part()" {{ (old('patient_type') == 0) ? 'checked' : '' }}>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;مريض موجود</label>
  <label><input type="radio" name="patient_type" value="1" id="default_val_check" onclick="new_patient_part()" {{ (old('patient_type') == 1) ? 'checked' : '' }}>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;مريض جديد</label>
...
@if(old('patient_type') == 0)
<div id="current_patient" style="display:{{ (old('patient_type') == 0) ? 'block' : 'none' }};">
@elseif(old('patient_type') == 1)
<div id="current_patient" style="display:{{ (old('patient_type') == 1) ? 'none' : 'block' }};">
@else
...
<div id="old_patient" style="display:{{ (old('patient_type') == 1) ? 'block' : 'none' }};">

<div class="col-md-4">
    <div class="form-group">
      <label for="email">رقم الملف</label><br>
      <input type="text" class="form-control"  name="patient_file_no" value="{{ old('patient_file_no') }}">
      @error('patient_file_no')
...
<div class="col-md-4">
    <div class="form-group">
      ...
      <input type="text" class="form-control"  name="patient_phone" value="{{ old('patient_name') }}">
      @error('patient_phone')
        <div class="alert alert-danger">{{ $message }}</div>
      @enderror
    </div>
...
@if($errors->any())
<input type="hidden" class="form-control"  id="is_error" value="1" disabled>
<input type="hidden" class="form-control"  id="old_pro" value="{{json_encode(old('multi_product'))}}" disabled>
<input type="hidden" class="form-control"  id="old_quant" value="{{json_encode(old('multi_amount'))}}" disabled>
@else
<input type="hidden" class="form-control"  id="is_error" value="0" disabled>
@endif
...
<br><!--<button type="button" name="add" id="add" class="btn btn-success add"><i class="fa fa-plus"></i></button><br><br>-->
  ...
  <div class="frmSearch">
<input type="text" id="search-box" onkeyup="search();" placeholder="بحث بالاسم" />
...
<script type="text/javascript">
  ...
  function check_error(){
    var check = document.getElementById("is_error").value;
   
    if(check == 1 ){
      var old_pro = JSON.parse(document.getElementById("old_pro").value);
      var old_quant = document.getElementById("old_quant").value;

      console.log(old_pro);
      for(var j = 0 ; j < old_pro.length ; j++){
        $.ajax({
          type: 'GET',
          url:  "{{url('ajax_get')}}/"+ old_pro[j],
