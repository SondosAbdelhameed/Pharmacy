...
class DoctorController extends Controller
{
    ...
    public function add_new_prescription(Request $request){
        ...
        $doctor = Doctor::where('user_doctor_id',auth()->user()->id)->first();
        $patient_id = null;
        if ($request->patient_type == 0) {
            $patient_id = $request->patient_id;
        }else{

            $this->validate($request, [
                'patient_file_no' => 'required|unique:patients,folder_number',
    ...
    public function ajax_get($search_val){

        $pro = Item::with('product')->find($search_val);
        if($pro == ''){
            $response['error'] = true;
            $response['status'] = 0;
            $response['message'] = "No Product"; 
    ...
    public function show_prescription_detail($id){
        $pres = Prescription::with('items')->with('patient')->find($id);
